...
import {
  ...
  CheckCircle2, Star, Users, Sparkles,
} from "lucide-react";
import {
...
export default async function HomePage() {
  const [schoolsRaw, scholarshipsRaw, jobsRaw, stats] = await Promise.all([
    getFeaturedSchools(6), getFeaturedScholarships(3), getFeaturedJobs(4), getStats(),
  ]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground overflow-x-hidden">
        ...
        <div className="container relative z-10 px-6 mx-auto max-w-[1280px] pt-10 pb-20">
          <div className="max-w-[820px] mx-auto text-center space-y-8">
            ...
            >
              ...
              <div className="relative rounded-[28px] border border-border/60 bg-card shadow-2xl shadow-black/10 overflow-hidden">
                {/* Browser chrome */}
                ...
                <div className="grid grid-cols-3 gap-4 p-6 bg-gradient-to-br from-background to-muted/30">
                  {[
                    ...
                    <div key={i} className="ns-card bg-card p-4 space-y-1">
                      <p className="text-xs text-muted-foreground font-medium">{card.label}</p>
                      <p className={`text-2xl font-bold ${card.color}`}>{card.value}</p>
                      <p className="text-[11px] text-muted-foreground">{card.sub}</p>
                    </div>
                  ))}
                  <div className="col-span-3 ns-card bg-card p-4">
        ...
        <div className="container px-6 mx-auto max-w-[1280px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative">
            ...
            {[
              ...
              <div
                key={i}
                className="group relative flex flex-col items-center text-center p-8 rounded-[32px] bg-card border border-border/50 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all duration-500"
              >
                {/* Step number */}
      ...
      ═══════════════════════════════════════════════════════════ */}
      <section className="py-24 md:py-36 bg-card/30 border-y border-border/50">
        <div className="container px-6 mx-auto max-w-[1280px]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {/* AI Match card demo */}
            <div className="space-y-4">
              <div className="ns-card bg-card p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="ns-card bg-card p-4 flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {schools.map((school, i) => (
              <SchoolCard
                ...
                type={school.type} logoUrl={school.logoUrl}
                description={school.description} isVerified={true}
              />
            ))}
          </div>
        </div>
      ...
      <section className="py-24 md:py-36 bg-primary relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-white/5 rounded-full blur-[120px] animate-float-slow" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-white/5 rounded-full blur-[100px] animate-float" />
        <div className="container relative z-10 px-6 mx-auto max-w-[1280px]">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Proven at scale.</h2>
            <p className="text-white/60 text-lg">Numbers that define our impact.</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { label: "Universities",        count: stats.totalSchools,       icon: Building2, suffix: "+" },
              { label: "Strategic Partners",  count: 120,                      icon: Shield,    suffix: "+" },
              { label: "Active Scholarships", count: stats.totalScholarships,  icon: Banknote,  suffix: "+" },
              { label: "Job Postings",        count: stats.totalJobs,          icon: Briefcase, suffix: "+" },
            ].map((stat, i) => (
              <div key={i} className="group text-center space-y-4">
                <div className="w-14 h-14 rounded-2xl bg-white/10 group-hover:bg-white/20 transition-colors flex items-center justify-center mx-auto">
                ...
                <p className="text-sm font-semibold text-white/50 uppercase tracking-widest">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
